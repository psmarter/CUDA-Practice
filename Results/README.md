...
> 本目录收录了 CUDA-Practice 项目全部 15 个主题模块的**真机运行报告与结果整理**，涵盖正确性验证、Kernel 耗时、有效带宽、计算吞吐 (GFLOPS/TFLOPS)、加速比等关键指标；个别大尺寸样例、可选依赖模块和失败实验会在对应章节中单独标注限制条件。
...
报告整体尽量按以下维度组织，但并非每个样例都完整覆盖全部项目：

1. **正确性验证** — 在可承受规模下进行 CPU 参考实现 vs GPU 结果逐元素容差比对
2. **Kernel 计时** — `cudaEventRecord` 多轮迭代取平均；极短 Kernel 可能因打印精度显示为 `0.00 ms`
3. **内存安全** — 结合 `compute-sanitizer` 检查越界/竞争，是否执行以各分报告为准
4. **性能剖析** — 结合 Nsight Compute (`ncu`) 采集 SM/DRAM 吞吐与寄存器利用率，是否执行以各分报告为准
...
> 注：汇总页仅保留最具代表性的条目。极短 Kernel 的 `0.00 ms` 多为打印精度所致；涉及可选依赖、失败实验或待复验实现时，请以对应专题报告中的限制说明为准。
