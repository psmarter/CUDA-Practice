---
title: CUDA-Practice：11 算子融合、PagedAttention 与 Continuous Batching
tags:
  - CUDA
  - GPU编程
  - 并行计算
...
>
> **本篇在系列中的位置**：承接 [05 大模型算子与注意力归一化](/posts/cb29461c/) 与 [07 量化、半精度与整数推理](/posts/ef325d2f/)、[10 访存优化与共享内存冲突](/posts/5b6f891d/) 关于「单算子算力/带宽」的优化，本篇站在 **LLM 推理系统视角**，回答「当算子本身已很快时，如何通过 KV Cache、算子融合和批处理策略，把整个服务端吞吐拉满」。后续 [08 多流、图执行与扩展开发](/posts/b1c0c6a3/) 的 Multi-Stream / CUDA Graphs、[15 多卡通信与全归约](/posts/b599e19f/) 的跨卡通信，会把这些内核/系统优化扩展到多流、多卡级别。
...
| 文章 | 与本篇的衔接 |
|------|--------------|
| [05 大模型算子与注意力归一化](/posts/cb29461c/) | 先理解单次自注意力 / Norm / FlashAttention 的算子级优化与带宽瓶颈，再看本篇这些算子在推理系统中的部署问题 |
| [07 量化、半精度与整数推理](/posts/ef325d2f/) | 本篇默认你已经掌握 FP16/INT8 的混合精度和带宽收益，便于理解推理时 KV Cache/Activation 的内存压力来源 |
| [10 访存优化与共享内存冲突](/posts/5b6f891d/) | 本篇的 Kernel Fusion、PagedAttention、Continuous Batching 都依赖良好的 Global 合并访问与合理的 Shared 使用 |
| [13 性能分析、屋顶线与占用率](/posts/803b94d6/) | 诊断篇中对 $I$ 与带宽天花板的分析，是本篇判断 Memory Bound / Capacity Bound 的基础 |

### 推荐后续

| 文章 | 与本篇的衔接 |
|------|--------------|
| [08 多流、图执行与扩展开发](/posts/b1c0c6a3/) | 将本篇的推理算子与 KV Cache 放入多流和 CUDA Graphs 的系统级调度框架中 |
| [14 模板矩阵乘与代数布局](/posts/f1b57921/) | 即便已经消除了 Padding 和中间回写，底层 GEMM / Attention 内核仍然需要 CUTLASS 级优化来压榨硬件 |
| [15 多卡通信与全归约](/posts/b599e19f/) | 在多卡环境下继续扩展本篇的优化思路，从单卡推理走向多卡分布式推理 |
