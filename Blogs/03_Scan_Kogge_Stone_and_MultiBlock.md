...
> **Brent-Kung Up-sweep 各阶段活跃线程比例**（N=1024 的情况）：
>
> | Up-sweep 轮次 | 活跃步长 Stride | 活跃线程数 | 占总线程比例 | Warp 利用率 |
> | :---: | :---: | :---: | :---: | :---: |
> | 第 1 轮 | 1 | 512 | 50% | 100%（两两配对，Warp 全满）|
> | 第 2 轮 | 2 | 256 | 25% | 50% |
> | 第 5 轮 | 16 | 32 | 3.1% | 仅 1 Warp 工作 |
> | 第 8 轮 | 128 | 4 | 0.4% | Warp 内 4/32 活跃（12.5%）|
> | **第 10 轮** | **512** | **1** | **0.1%** | **Warp 内 1/32 活跃（3.1%）** |
>
> 最后几步只有 1 个线程在工作，却占用了整个 Block 的所有同步开销和 `__syncthreads()` 路障。这与 Kogge-Stone 的"每步所有线程均有效工作"形成了鲜明对比——这才是 BK 在 GPU 上输给 KS 的根本原因，而非工作量（BK 的工作量反而更少）。
